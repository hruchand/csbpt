...
void csbInsert64(struct CSBINODE64* root,struct CSBINODE64* parent, int childIndex,struct LPair new_entry, int* new_key, void** new_child) {
	...
	if (IsLeaf(root)) {    // This is a leaf node
		...
		if (((struct BPLNODE64*)root)->d_num < 6) {
			////we still have enough space in this leaf node.
			for (i=((struct BPLNODE64*)root)->d_num; i>l; i--)

				((struct BPLNODE64*)root)->d_entry[i]=((struct BPLNODE64*)root)->d_entry[i-1];//right shift
...
int csbDelete64(struct CSBINODE64* root,struct LPair del_entry) {
	...
	while (!IsLeaf(root)) { // if the current root is not a leaf. Root a node.
...
void csbPrint64(struct CSBINODE64* root){
	int i,j,k,l;

	for (i = 0; i<14 ;i++){
		//printf("inside print");
		if(root->d_keyList[i]!= 0)
			printf("%d\t",root->d_keyList[i]);
	...
	if(root->d_firstChild != NULL){
		printf("\n");
		struct CSBINODE64* first_child = root->d_firstChild;
		for (j = 0; j<14 ;j++){
			if(first_child->d_keyList[j]!= 0)
				printf("%d\t",first_child->d_keyList[j]);
		}
		struct BPLNODE64* second_child = first_child->d_firstChild;
		printf("\n");
		for (k = 0; k<2 ;k++){
			//if (second_child->d_entry[k].d_key!=0)
			printf("%d\t",second_child->d_entry[k].d_key);
		}
		while (second_child->d_next!= NULL){
			second_child = second_child->d_next;
			for(l=0;l<2;l++){
				printf("%d\t",second_child->d_entry[l].d_key);
			}
		}
